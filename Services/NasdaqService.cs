...
namespace MarketRadar.Services
{
    public class NasdaqService
    {
        ...
        private readonly string _nasdaqUrl = string.Empty;

        public NasdaqService(HttpClient http, IOptions<Setting> setting)
        {
            _http = http;
            _nasdaqUrl = setting.Value.NasdaqUrl;
        }

        public async Task<PriceTrend> GetNasdaqDataAsync()
        {
            var request = new HttpRequestMessage(HttpMethod.Get, _nasdaqUrl);
            request.Headers.Add(
                "User-Agent",
